...
class DrinkBloc implements BlocBase {
  ...
  Function(int) get _inSelectedAmount =>
      _selectedDrinkAmountController.sink.add;
  Stream<int> get outSelectedAmount => _selectedDrinkAmountController.stream;

  Stream<int> get outDrinksAmount {
    return outDrinks.map((drinks) => drinks.fold<int>(
        0, (totalAmount, drink) => totalAmount + drink.amount));
  }

  Future<void> init() async {
    final drinkStream =
        await FirestoreDrinkService.getDrinksStream(DateTime.now());
    _drinkStreamSubscription = drinkStream.listen((querySnapshot) {
      _drinksToday = querySnapshot.documents
          .map((doc) => Drink.fromDb(doc.data, doc.documentID))
          .toList();
